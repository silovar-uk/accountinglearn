renderHome = function renderDeepHome() {
  ...
  const caseNumber = String(Number(featured.releaseOrder || featured.metadata?.releaseOrder || 1)).padStart(2, "0");
...
renderCaseView = function renderDeepCaseView(caseId, rawPageIndex) {
  ...
  const caseNumber = String(Number(data.releaseOrder || data.metadata?.releaseOrder || 1)).padStart(2, "0");
  return `<main class="main case-main deep-case-layout">
    <aside class="case-outline" aria-label="ケースのページ一覧"><div class="outline-heading"><span>CASE ${caseNumber}</span><strong>調査ファイル</strong></div>${renderCaseOutline(data, pageIndex)}</aside>
