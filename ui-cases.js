...
function renderCaseCard(data, index) {
  ...
  return `<article class="card case-card ${complete ? "complete" : started ? "in-progress" : ""}">
    ...
    <div class="case-card-body"><div class="case-card-status"><span>${complete ? "COMPLETED" : started ? "IN PROGRESS" : "AVAILABLE"}</span><small>${data.estimatedMinutes}分</small></div><h3>${escapeHtml(data.title)}</h3><p>${escapeHtml(data.subtitle)}</p>${started ? `<div class="mini-progress"><i style="width:${progress}%"></i></div>` : ""}</div>
...
function renderPlannedCases() {
  const planned = [["02","倉庫に眠るヒット商品","在庫と粗利益"],["03","売上が伸びるほど苦しくなる会社","顧客単位の採算"],["04","工場は忙しいのに、利益が消えた","製品別原価"],["05","満席の店から撤退すべきか","撤退判断"],["06","観客は増えた。それでも資金が足りない","スポーツクラブの資金繰り"],["07","伸ばす事業と、やめる事業","事業ポートフォリオ"],["08","この会社を買うべきか","M&A・事業再生"]];
  return planned.map(([no,title,subtitle]) => `<article class="card case-card planned"><div class="case-number">${no}</div><div class="case-card-body"><div class="case-card-status"><span>PLANNED</span><small>準備中</small></div><h3>${title}</h3><p>${subtitle}</p></div><span class="case-lock">${navIcon("lock", 19)}</span></article>`).join("");
}
